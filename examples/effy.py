...
def build_css(styles):
    css = []
    # Three selectors just to make the rule very specific.
    inner = f'''body .cls_primary_outer .cls_primary_inner {{
                    width: {styles['width']};
...
class LatLongDistCalculator(flx.Widget):
    ...
    def init(self):
        super().init()
        with flx.VBox(css_class='cls_primary_inner'):
            with flx.FormLayout():
                self.in_lat1 = \
        ...
        if None in (lat1_rad, long1_rad, lat2_rad, long2_rad):
            ...
            dist_m = \
              latlong_to_dist_m_vincenty_js(lat1_rad, long1_rad,
                                            lat2_rad, long2_rad)
            self.lbl_distance.set_text(dist_str_js(dist_m))
