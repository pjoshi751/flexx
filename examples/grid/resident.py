        ...
        with flx.GridLayout(ncolumns=3) as self.grid:
            ...
            for item in elements:
                if item[0] == 'button':
                    ...
                    self.lines.append(flx.LineEdit(text=item[2]))
                    flx.Widget()
            flx.Widget(flex=(0, 1))
            flx.Widget()
            flx.Widget()
    ...
    def init(self):
        with flx.VBox():
            with flx.HBox():
                ...
                flx.ImageWidget(flex=1, source='https://www.omidyarnetwork.in/wp-content/uploads/2019/05/mosip.png', css_class='logo', maxsize=(80, 80))
            with flx.HBox():
                ...
                with flx.StackLayout(flex=1) as self.stack:
                    ...
                    '''
                    '''
                    with flx.GridLayout(ncolumns=3) as w:
                        self.stack_elements.append(w)
                        flx.Label(text='', flex=(0, 0))
                        flx.Label(text='RID status', flex=(0, 0), css_class='subtitle')
                        flx.Widget(flex=(1, 0))
                        flx.Label(text='RID', flex=(0, 0), css_class='col_0')
                        self.rid = flx.LineEdit(text='')
                        flx.Widget()
                        flx.Label(text='', flex=(0, 0))
                        self.rid_submit = flx.Button(text='Submit')
                        flx.Widget()
